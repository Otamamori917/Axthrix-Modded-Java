...
public class Extension {
    ...
    public void update(Unit unit){
        IkatusaUnitType type = (IkatusaUnitType)unit.type;
        Length = type.bLength;
                    ...
                    for (Team t : Team.all)
                        if (t != unit.team && t.active())
                            Units.nearby(t, unit.x, unit.y, 2, other -> {
                                other.apply(StatusEffects.none, 60*8);
                            });
                }
            }
        });
    ...
    public void updateTargetPosition(float tx, float ty){
        ...
        for(int i = idx1 + 1; i < size; i++){
            ...
            vr -= Mathf.clamp(ExtensionUtils.angleDistSigned(lr, Angles.angle(lx, ly, targetX, targetY)) * 0.02f, -1f, 1f);
            lr += vr;
            vr *= 1f - 0.05f;
            vel[i2] = vr;
            vel[i2 + 1] = lr;
