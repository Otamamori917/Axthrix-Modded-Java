...
public class TemperatureGaugeUI {
    ...
    public void build(){
        ...
        container.visible(() -> {
            ...
            float temp = 0f;
            if(u != null){
                temp = TemperatureBulletType.getTemperatureUnit(u);
            }else if(control.input.config.getSelected() instanceof Turret.TurretBuild tb){
                temp = TemperatureBulletType.getTemperatureBuilding(tb);
            }
        ...
        container.update(() -> {
            ...
            if(unit != null && unit.isPlayer()){
                temp = TemperatureBulletType.getTemperatureUnit(unit);
                label = "Unit";
    ...
    private void buildGauge(float temperature, String label){
        container.table(t -> {
            ...
            String tempText = temperature > 0 ?
                    "[red]+" + Strings.fixed(temperature, 1) + "°" :
                    "[cyan]" + Strings.fixed(temperature, 1) + "°";
