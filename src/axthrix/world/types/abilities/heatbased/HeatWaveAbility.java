...
public class HeatWaveAbility extends Ability{
	...
	public float range;
	public float damage;
	public float temperaturePerHit = 15f; // Heat applied per wave
	...
	public void addStats(Table t){
		...
		t.row();
		t.add("[lightgray]Temperature: [red]+" + buildVal +"heat");
		t.row();
	}
	...
	public void init(UnitType type){
		super.init(type);
		buildVal = TempUnit.format(temperaturePerHit);
		...
		if(maxSpeed > 0 && unit.vel().len2() > maxSpeed){
			...
			if(all.any()){
				...
				for(Unit u : all){
					...
					// Apply heat
					TemperatureLogic.applyTemperatureUnit(u, temperaturePerHit);
